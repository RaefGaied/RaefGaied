...
import { sampleWithRequiredData } from '../email-template-configuration.test-samples';
import { EmailTemplateConfigurationService } from '../service/email-template-configuration.service';

import { EmailTemplateConfigurationComponent } from './email-template-configuration.component';
import SpyInstance = jest.SpyInstance;

describe('EmailTemplateConfiguration Management Component', () => {
  let comp: EmailTemplateConfigurationComponent;
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [EmailTemplateConfigurationComponent],
      providers: [
        provideHttpClient(),
        {
      ...
      .overrideTemplate(EmailTemplateConfigurationComponent, '')
      .compileComponents();
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.emailTemplateConfigurations?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to emailTemplateConfigurationService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getEmailTemplateConfigurationIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getEmailTemplateConfigurationIdentifier).toHaveBeenCalledWith(entity);
